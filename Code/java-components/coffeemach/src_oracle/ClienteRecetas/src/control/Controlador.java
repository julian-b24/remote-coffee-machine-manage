...
import interfaz.InterfazRecetas;
...
import servicios.ServicioClienteRecServidor;

public class Controlador implements Runnable {

	@Reference
	private ServicioClienteRecServidor servicioClienteRecServidor;

	public void setServicioClienteRecServidor(
			ServicioClienteRecServidor servicioClienteRecServidor) {
		...
		iR.getBtnRIC().addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!iR.getTextFieldAsociacion().getText().isEmpty()) {
					...
					servicioClienteRecServidor.registrarReceta_Ingrediente(
